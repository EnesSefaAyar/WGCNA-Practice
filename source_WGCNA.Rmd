...
df_WGCNA <- mtx[, colMeans(is.na(mtx)) < 0.84]

res <- rcorr(df_WGCNA, type = "pearson")
cor_df <- res$r

hist(colMeans(is.na(mtx)))
```
...
dynamicMods <- cutreeDynamic(geneTree, minClusterSize = 50)
...
cluster_members_WGCNA <- rownames(df_WGCNA)[clusters_WGCNA == 3]
