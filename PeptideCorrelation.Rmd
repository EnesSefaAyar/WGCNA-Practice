...
l = length(unique(all_cor_df$protein))
i <- 0
for (protein in unique(all_cor_df$protein)) {
...
ggplot() + aes(all_cleaned$r)+ geom_histogram(binwidth = 0.1,
                                        colour = "black", fill = "white") +
  ylim(0, 2000)
...
cor_old <- res$r
diag(cor_old) <- NA
cor_old <- cor_old[rownames(cor_old) %in% prots_altered, ]

res <- rcorr(t(df_new), type = "pearson")
cor_new <- res$r
diag(cor_new) <- NA
cor_new <- cor_new[rownames(cor_new) %in% prots_altered, ]
...
ggplot(rbind(old_df, new_df), aes(y = r, x = as.factor((type)))) + 
    geom_boxplot() +
    geom_jitter(color = "black", size = 0.05, alpha = 0.4) +
    theme_classic()
