...
```{r}
# Load the RNA-seq dataset of normalized counts. This includes 12 samples, 
# 3 females and 3 males at E.12.5 and and 3 females and 3 males at E.16.5.
# The datasat includes 14,089 genes. 

dir <- "~/WGCNA-Practice/data/GSE117590_All_FPKM.txt"

df <- read.csv(dir, sep = "\t")

datExpr0 <-as.data.frame(df)
rownames(datExpr0)<-datExpr0$GeneID
datExpr0 <- datExpr0[, -c(1, 26, 27, 28, 29, 30)]

datExpr0 <- log1p(as.matrix(datExpr0))
...
We produce a sample dendrogram to visualize potential outliers. 
We set the threshold at height = 30 because it removes the highest sample, F2_S2
...
Now, we eliminate the sample and double check. To eliminate the sample we keep
all that are grouped at least in groups of 2 and below the height of 30. 
The variable clust contains the information on which group is each sample. 
Filter by the ones you want to keep. In this case we keep all that are not
listed in group 0.
...
par(cex = 0.6);
par(mar = c(0,4,2,0))
plot(sampleTree, main = "Sample clustering without outliers",
     sub = "", xlab = "", cex.lab = 1.5, cex.axis = 1.5, cex.main = 2)
...
```{r}
traitData <- read.csv(MouseTraits_12_16)
dim(traitData)
names(traitData)
